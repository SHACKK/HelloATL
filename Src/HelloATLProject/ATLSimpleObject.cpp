...
STDMETHODIMP_(HRESULT __stdcall) CATLSimpleObject::Initialize(LPCITEMIDLIST pidFolder, LPDATAOBJECT pDataObj, HKEY hProgID)
{
	...
	for (UINT uFile = 0; uFile < uNumFiles; uFile++)
	{
		...
		m_vecFiles.push_back(szFile);
	}

	GlobalUnlock(stg.hGlobal);
	ReleaseStgMedium(&stg);

	return (m_vecFiles.size() > 0 ? S_OK : E_INVALIDARG);
}

STDMETHODIMP_(HRESULT __stdcall) CATLSimpleObject::GetCommandString(UINT_PTR idCmd, UINT uFlags, UINT* pwReserved, LPSTR pszName, UINT cchMax)
{
	USES_CONVERSION;
	// idCmd 검사
...
STDMETHODIMP_(HRESULT __stdcall) CATLSimpleObject::InvokeCommand(LPCMINVOKECOMMANDINFO pCmdInfo)
{
	...
	case 0:
	{
		for (auto& iter : m_vecFiles)
			MessageBox(pCmdInfo->hwnd, iter.c_str(), TEXT("HELLO"), MB_ICONINFORMATION);
...
STDMETHODIMP_(HRESULT __stdcall) CATLSimpleObject::QueryContextMenu(HMENU hMenu, UINT uMenuIndex, UINT uidFirstCmd, UINT uidLastCmd, UINT uFlags)
{
	...
	InsertMenu(hMenu, uMenuIndex, MF_BYPOSITION, uidFirstCmd, TEXT("Akdan으로 검사하기"));
	//SetMenuItemBitmaps로 bmp 이미지 추가 가능
